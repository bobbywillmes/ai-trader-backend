...
## Securities and dependencies

The Securities catalog preserves URL-backed search, asset type, classification, enabled-state, usage, page-size, pagination, and sorting behavior. Wide containers use a concise table; compact and narrow containers use summary rows and cards. The summary exposes the authoritative subscription count supplied by the API, while details direct operators to the existing symbol workspace for dependent strategies, exit profiles, and activity.

The current security contract exposes symbol, name, asset type, sector, industry, enabled state, and subscription usage. The UI does not infer exchange, provider identity, currency, or tradable state when those fields are absent. Create semantics and symbol uniqueness remain server-authoritative. On the symbol detail route, subscription records become labeled cards on narrow containers instead of retaining the legacy horizontally scrolling table; consequential enable/disable behavior and existing API calls are unchanged.
