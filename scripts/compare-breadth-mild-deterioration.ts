...
import { compareBreadthMildDeteriorationConfirmation } from '../src/dev/breadth-mild-deterioration-comparison.js';
import { renderBreadthMildDeteriorationReport } from '../src/dev/breadth-mild-deterioration-report.js';
...
try {
  let comparison;
  try {
    comparison = await compareBreadthMildDeteriorationConfirmation({ from, to });
  } catch (error) {
  ...
  const markdown = renderBreadthMildDeteriorationReport(comparison);
  if (outputPath) {
