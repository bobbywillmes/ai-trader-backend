...
export function LifecycleRepairCaseDetail({ item, onApply, onDiagnoseAgain, onDecision, onApplyAction }: { item: LifecycleRepairCase; onApply: () => void; onDiagnoseAgain: () => void; onDecision?: (actionId: number, revision: number, decision: "APPROVE" | "REFUSE", reason: string) => void; onApplyAction?: (actionId: number, revision: number, reason: string, confirmation: string) => void }) {
  ...
  if (item.repairType === "REPAIR_HISTORICAL_ENTRY_LIFECYCLE") {
    ...
    return <Stack gap="lg" data-testid="historical-entry-repair-case-detail">
      ...
      <TextInput label="Required approval or refusal reason" value={actionReason} onChange={(event) => setActionReason(event.currentTarget.value)} />
      {(item.actions ?? []).map((action) => { const confirmation = action.actionType === "TERMINALIZE_ORDER_LIFECYCLE" ? "TERMINALIZE HISTORICAL ORDER LIFECYCLE" : "LINK HISTORICAL ENTRY LIFECYCLE"; return <Card withBorder key={action.id}><Stack><Group justify="space-between"><Title order={3}>{titleCase(action.actionType)}</Title><Group><Badge>{action.classification}</Badge><Badge>{action.status}</Badge></Group></Group><Text size="sm">Action fingerprint: <Code>{action.actionFingerprint}</Code></Text><RawDisclosure label="Backend-proposed field mutations" value={action.proposedMutationsJson} /><Text size="xs">Preview expires {date(item.expiresAt)} · revision {action.revision}</Text>{action.decisionReason && <Text>Decision: {action.decisionReason}</Text>}{action.status === "PROPOSED" && <Group><Button disabled={!actionReason.trim()} onClick={() => onDecision?.(action.id, action.revision, "APPROVE", actionReason)}>Approve</Button><Button color="red" variant="light" disabled={!actionReason.trim()} onClick={() => onDecision?.(action.id, action.revision, "REFUSE", actionReason)}>Refuse</Button></Group>}{action.status === "APPROVED" && <><TextInput label={`Type ${confirmation}`} value={actionConfirmation} onChange={(event) => setActionConfirmation(event.currentTarget.value)} /><Button color="red" disabled={!actionReason.trim() || actionConfirmation !== confirmation || item.tradingAccount.environment !== "PAPER"} onClick={() => onApplyAction?.(action.id, action.revision, actionReason, actionConfirmation)}>Apply approved action</Button></>}</Stack></Card>; })}
