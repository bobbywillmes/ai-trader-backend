...
function HistoricalExecutionResult({ execution, action }: { execution: LifecycleRepairExecution; action: LifecycleRepairAction }) {
  const validation = object(execution.validationJson); const before = object(execution.beforeJson); const after = object(execution.afterJson);
  const beforeOrder = object(before.brokerOrder); const afterOrder = object(after.brokerOrder); const beforeIntent = object(before.orderIntent); const afterIntent = object(after.orderIntent);
  const beforeActivities = array(before.activities).map(object); const afterActivities = array(after.activities).map(object);
  const succeeded = execution.result === "SUCCEEDED"; const structurallyValid = validation.valid === true;
  const identityConsistent = ["id", "tradingAccountId", "broker", "brokerOrderId", "clientOrderId"].every((key) => beforeOrder[key] === afterOrder[key]) && beforeIntent.id === afterIntent.id && beforeIntent.tradingAccountId === afterIntent.tradingAccountId;
  const noPositionRelationshipChanged = beforeOrder.trackedPositionId === afterOrder.trackedPositionId && beforeIntent.trackedPositionId === afterIntent.trackedPositionId && beforeActivities.every((activity) => afterActivities.find((candidate) => candidate.id === activity.id)?.trackedPositionId === activity.trackedPositionId);
  const targetPositionId = afterOrder.trackedPositionId; const proposal = object(action.proposedMutationsJson); const eligibleActivityIds = array(proposal.brokerActivityIds);
  const eligibleActivities = afterActivities.filter((activity) => eligibleActivityIds.includes(activity.id));
  const activitiesLinked = eligibleActivityIds.length > 0 && eligibleActivities.length === eligibleActivityIds.length && eligibleActivities.every((activity) => activity.trackedPositionId === targetPositionId);
  const provenanceRecorded = eligibleActivities.length > 0 && eligibleActivities.every((activity) => activity.trackedPositionLinkSource === "historical_order_lifecycle_repair");
  const unchangedProtectedState = validation.financialValuesChanged === false && validation.exposureChanged === false;
  const pending = action.status === "APPLIED";
  return <Card withBorder className={classes.execution} data-result={execution.result}><Stack gap="sm"><Group justify="space-between"><Title order={4}>Execution {execution.id}</Title><Badge size="lg" color={succeeded ? "teal" : "red"}>{execution.result}</Badge></Group>
    {succeeded ? <SimpleGrid cols={{ base: 1, sm: 2 }}>
      {action.actionType === "TERMINALIZE_ORDER_LIFECYCLE" ? <><Status pass={afterOrder.status === "filled"}>BrokerOrder reached filled status</Status><Status pass={afterIntent.status === "filled"}>OrderIntent reached filled status</Status><Status pass={identityConsistent}>Order and intent identity remained consistent</Status><Status pass={noPositionRelationshipChanged}>No position relationship changed</Status></> : <><Status pass={targetPositionId !== null && targetPositionId !== undefined && afterIntent.trackedPositionId === targetPositionId}>OrderIntent linked to backend-selected TrackedPosition</Status><Status pass={targetPositionId !== null && targetPositionId !== undefined}>BrokerOrder linked to the same TrackedPosition</Status><Status pass={activitiesLinked}>Eligible BrokerActivity rows linked consistently</Status><Status pass={provenanceRecorded}>Link provenance recorded</Status></>}
      <Status pass={unchangedProtectedState && structurallyValid}>No financial, exposure, timestamp, or raw broker payload changed</Status><Status pass={validation.brokerWrites === 0}>Broker mutation performed: {validation.brokerWrites === 0 ? "NO" : "YES"}</Status><Status pass={structurallyValid}>Structural validation: {structurallyValid ? "PASS" : "FAIL"}</Status>
    </SimpleGrid> : <Alert color="red" title="Execution failed">{text(object(execution.failureJson).message, "The repair was rolled back without a successful mutation.")}</Alert>}
    {pending && <Alert color="blue">Authoritative reconciliation verification: pending.</Alert>}
    {action.status === "VERIFIED" && <Alert color="teal">Authoritative reconciliation verification: VERIFIED.</Alert>}
    <SimpleGrid cols={{ base: 1, sm: 2, lg: 3 }}><SummaryItem label="Actor"><Text>{execution.executedByUser?.name ?? execution.executedByUser?.email ?? "System Owner"}</Text></SummaryItem><SummaryItem label="Executed"><Text>{date(execution.executedAt)}</Text></SummaryItem><SummaryItem label="Reason"><Text>{execution.reason}</Text></SummaryItem></SimpleGrid>
    <RawDisclosure label="Raw execution details" value={{ beforeJson: execution.beforeJson, afterJson: execution.afterJson, validationJson: execution.validationJson, failureJson: execution.failureJson }} />
  </Stack></Card>;
}
...
export function LifecycleRepairCaseDetail({ item, onApply, onDiagnoseAgain, onDecision, onApplyAction, onReconsider }: { item: LifecycleRepairCase; onApply: () => void; onDiagnoseAgain: () => void; onDecision?: (actionId: number, revision: number, decision: "APPROVE" | "REFUSE", reason: string) => void; onApplyAction?: (actionId: number, revision: number, reason: string, confirmation: string) => void; onReconsider?: (actionId: number, revision: number, reason: string) => void }) {
  ...
  if (item.repairType === "REPAIR_HISTORICAL_ENTRY_LIFECYCLE") {
    ...
    return <Stack gap="lg" data-testid="historical-entry-repair-case-detail">
      <Card withBorder><Stack><Group justify="space-between"><div><Text size="xs" c="dimmed">HISTORICAL ENTRY LIFECYCLE CASE {item.id} · GENERATION {item.generation}</Text><Title order={2}>{text(order.symbol)} historical BUY lifecycle</Title></div><Group><Badge>{item.tradingAccount.environment}</Badge><Badge color="orange">{item.confidence}</Badge></Group></Group><Text>At preview time, stored full-fill evidence showed these incomplete local components: {components.join(", ")}.</Text><Alert color="blue" title="No broker or exposure impact">Repair performs no broker calls or writes and changes no position quantity, status, price, P&amp;L, timestamps, or current exposure.</Alert></Stack></Card>
      ...
      <Card withBorder><Stack><Title order={3}>Candidate positions and predicates</Title>{candidates.length ? candidates.map((candidate) => { const comparison = object(candidate.comparison); return <Card withBorder key={text(candidate.trackedPositionId)}><Stack gap="xs"><Text fw={700}>TrackedPosition {text(candidate.trackedPositionId)}</Text><SimpleGrid cols={{ base: 1, sm: 2 }}><SummaryItem label="Fill-derived expected price"><Text>{money(comparison.fillDerivedExpectedPrice)}</Text></SummaryItem><SummaryItem label="Candidate average-entry price"><Text>{money(comparison.candidateAverageEntryPrice)}</Text></SummaryItem><SummaryItem label="Absolute price difference"><Text>{money(comparison.absolutePriceDifference)}</Text></SummaryItem><SummaryItem label="Configured price tolerance"><Text>{money(comparison.priceTolerance)}</Text></SummaryItem><SummaryItem label="Fill completion to position open"><Text>{number(comparison.absoluteTimeDifferenceMs) === null ? "—" : `${number(comparison.absoluteTimeDifferenceMs)} ms`}</Text></SummaryItem><SummaryItem label="Configured time window"><Text>{number(comparison.timeToleranceMs) === null ? "—" : `${number(comparison.timeToleranceMs)} ms`}</Text></SummaryItem></SimpleGrid><Text size="sm"><b>Rejected predicates:</b> {array(candidate.rejectionReasons).join(", ") || "none"}</Text><RawDisclosure label="Candidate details" value={candidate} /></Stack></Card>; }) : <Text>No candidate position satisfied the discovery window.</Text>}</Stack></Card>
      {(item.actions ?? []).map((action) => { const reason = actionReasons[action.id] ?? ""; const enteredConfirmation = actionConfirmations[action.id] ?? ""; const confirmation = action.actionType === "TERMINALIZE_ORDER_LIFECYCLE" ? "TERMINALIZE HISTORICAL ORDER LIFECYCLE" : "LINK HISTORICAL ENTRY LIFECYCLE"; const disabled = item.expired || action.status === "SUPERSEDED"; return <Card withBorder key={action.id}><Stack><Group justify="space-between"><Title order={3}>{titleCase(action.actionType)} · generation {action.generation}</Title><Group><Badge>{action.classification}</Badge><Badge>{action.status}</Badge></Group></Group>{action.status === "VERIFIED" ? <Text><b>Recorded action reason:</b> {action.decisionReason ?? "Unavailable"}</Text> : <TextInput label="Required action reason" value={reason} onChange={(event) => { const value = event.currentTarget.value; setActionReasons((values) => ({ ...values, [action.id]: value })); }} />}<Text size="sm">Action fingerprint: <Code>{action.actionFingerprint}</Code></Text><RawDisclosure label="Backend-proposed field mutations" value={action.proposedMutationsJson} /><Text size="xs">Preview expires {date(item.expiresAt)} · revision {action.revision}</Text>{action.decisionReason && action.status !== "VERIFIED" && <Text>Decision: {action.decisionReason}</Text>}{action.reconsiderationReason && <Text>Reconsideration: {action.reconsiderationReason}</Text>}{action.status === "PROPOSED" && <Group><Button disabled={disabled || !reason.trim()} onClick={() => onDecision?.(action.id, action.revision, "APPROVE", reason)}>Approve</Button><Button color="red" variant="light" disabled={disabled || !reason.trim()} onClick={() => onDecision?.(action.id, action.revision, "REFUSE", reason)}>Refuse</Button></Group>}{action.status === "REFUSED" && <Button variant="light" disabled={!reason.trim()} onClick={() => onReconsider?.(action.id, action.revision, reason)}>Reconsider with fresh evidence</Button>}{action.status === "APPROVED" && <><TextInput label={`Type ${confirmation}`} value={enteredConfirmation} onChange={(event) => { const value = event.currentTarget.value; setActionConfirmations((values) => ({ ...values, [action.id]: value })); }} /><Button color="red" disabled={disabled || !reason.trim() || enteredConfirmation !== confirmation || item.tradingAccount.environment !== "PAPER"} onClick={() => onApplyAction?.(action.id, action.revision, reason, enteredConfirmation)}>Apply approved action</Button></>}{action.status === "APPLIED" && <Alert color="blue">Structurally applied. Authoritative reconciliation verification is pending.</Alert>}{action.executions.map((execution) => <HistoricalExecutionResult execution={execution} action={action} key={execution.id} />)}</Stack></Card>; })}
      {(item.actions ?? []).some((action) => action.status === "FAILED") && <Alert color="orange" title="Fresh preview required">The failed action and execution remain immutable. Rebuild authoritative evidence to create a new action generation with a new attempt key.</Alert>}
