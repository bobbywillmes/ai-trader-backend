        ...
        <Stack gap="md">
          ...
          {pipelineRuns.isError ? <Alert color="red">Recent run history could not be loaded.</Alert> : pipelineRuns.data?.data.length ? <ScrollArea.Autosize mah={460} type="auto" offsetScrollbars>
            <Table striped highlightOnHover stickyHeader>
              <Table.Thead><Table.Tr><Table.Th>Status</Table.Th><Table.Th>Started</Table.Th><Table.Th visibleFrom="md">Source</Table.Th><Table.Th visibleFrom="sm">Last stage</Table.Th><Table.Th>Duration</Table.Th></Table.Tr></Table.Thead>
              <Table.Tbody>{pipelineRuns.data.data.map((run) => <Table.Tr key={run.id}><Table.Td><Badge variant="light" color={run.status === "SUCCEEDED" ? "teal" : run.status === "RUNNING" ? "blue" : run.status === "PARTIAL" ? "yellow" : "red"}>{run.status}</Badge></Table.Td><Table.Td>{formatPipelineDate(run.startedAt)}</Table.Td><Table.Td visibleFrom="md">{run.source.replaceAll("_", " ")}</Table.Td><Table.Td visibleFrom="sm">{run.currentStage?.replaceAll("_", " ") ?? "—"}</Table.Td><Table.Td>{run.durationMs === null ? "—" : `${(run.durationMs / 1000).toFixed(1)}s`}</Table.Td></Table.Tr>)}</Table.Tbody>
            </Table>
