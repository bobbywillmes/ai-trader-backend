...
export function BindingEditor({ binding, token, pending, error, submit, close }: {
  ...
  const changed = !binding || enabled !== binding.enabled;
  ...
  const valid = changed && (binding ? acknowledged : sourceId && strategyId && validKey && catalogReady);
  return <Modal opened onClose={() => !pending && close()} title={binding ? "Edit strategy binding" : "Create strategy binding"} centered size="lg" closeOnClickOutside={!pending} closeOnEscape={!pending} withCloseButton={!pending}>
    <form onSubmit={event => {
      ...
      {binding ? <Alert color="blue"><Text size="sm">{catalogs.sourceName(binding.signalSourceId)} → {catalogs.strategyName(binding.strategyId)}</Text><Text size="sm" style={{ overflowWrap: "anywhere" }}>{binding.externalStrategyKey}</Text><Text size="xs" mt="xs">Source, Strategy, and external key are fixed for this binding.</Text></Alert> : <>
        ...
        <TextInput label="External strategy key" placeholder="Momentum Stock" required value={key} maxLength={200} onChange={event => setKey(event.currentTarget.value)} disabled={pending}
        ...
        <Text size="xs" c="dimmed">Example: momentum-stock or mean-reversion-etf. Use a stable algorithm identity, independent of accounts, risk profiles and revisions. Spaces become dashes and letters are lowercased. AI Trader manages revision numbers separately. Include a symbol only for a symbol-specific strategy.</Text>
      </>}
      {!binding && <Alert color="blue" title="Initial revision">Revision 1 will be created automatically.</Alert>}
