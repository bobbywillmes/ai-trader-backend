...
export function SignalDetails({ signal, token, navigate }: { signal: Signal; token: string | null; navigate: Navigate }) {
  ...
  return <Stack gap="lg">
    ...
    <section><Stack gap="sm"><Text fw={600}>Authority and routing</Text>
      <Text size="sm">Authority: {signal.routingRun?.authorityMode ?? signal.strategySignalRevision?.authorityMode ?? "EVIDENCE_ONLY"}</Text>
      <Text size="sm">Routing: {signal.routingRun?.status ?? "Not processed — historical Signal"} · Routes: {signal.routingRun?.routeCount ?? 0}</Text>
      {signal.routingRun?.stopReason && <Text size="sm">Stop reason: {signal.routingRun.stopReason}</Text>}
      <Text size="sm" c="dimmed">Routing records identity and configuration only. It makes no entry, exit, risk, or execution decision. This phase cannot trade.</Text>
      {signal.routingRun?.routes.map(route => <Card key={route.id} withBorder padding="sm" style={{ overflowWrap: "anywhere" }}><Stack gap="xs">
        <Anchor component={Link} to={`/trading-accounts/${route.tradingAccountId}`}>{route.targetSnapshot.tradingAccountName} · Account #{route.tradingAccountId}</Anchor>
        <Text size="sm">→ TradingAccountSubscription #{route.tradingAccountSubscriptionId}</Text>
        <Anchor component={Link} to={`/subscriptions?search=${encodeURIComponent(route.targetSnapshot.subscriptionKey)}`}>→ {route.targetSnapshot.subscriptionKey} · Subscription #{route.subscriptionId}</Anchor>
        <StrategyLink id={route.targetSnapshot.strategy.id} name={route.targetSnapshot.strategy.name} />
        <Text size="sm">→ {route.targetSnapshot.security.symbol} · Security #{route.targetSnapshot.security.id}</Text>
        <EvaluationDetails route={route} authority={signal.strategySignalRevision?.authorityMode ?? signal.routingRun?.authorityMode ?? 'EVIDENCE_ONLY'} />
      </Stack></Card>)}
    </Stack></section>
    {signal.eventFingerprint ? <section><Text size="sm" fw={600} mb="xs">Event fingerprint</Text><CopyValue value={signal.eventFingerprint} name="event fingerprint" /><Text size="xs" c="dimmed">Generated by AI Trader from canonical strategy-event identity.</Text></section> : signal.externalEventKey && <section><Text size="sm" fw={600} mb="xs">Legacy external event key</Text><CopyValue value={signal.externalEventKey} name="legacy event key" /></section>}
