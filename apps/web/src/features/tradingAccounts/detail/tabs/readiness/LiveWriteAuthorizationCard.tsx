...
function CapabilityCard({ account, token, capability, latest }: {
  ...
  const grantDisabledMessage = state.data?.deploymentCanWrite !== true
          ...
          : !item?.fingerprints
            ...
            : !reason.trim()
              ...
              : confirmation !== expectedConfirmation
                ...
                : capability === "ENTRY" && !expirationIsFuture
                  ? "Enter a valid future expiration."
                  : grant.isPending || revoke.isPending
                    ? "Approval operation is in progress."
                    : null;
  ...
  return <Card withBorder>
    ...
    {state.data?.deploymentCanWrite && <Stack gap="xs" mt="md">
      ...
      {!canGrant && grantDisabledMessage && <Text size="sm" c="dimmed" data-testid={`${capability.toLowerCase()}-grant-disabled-guidance`}>{grantDisabledMessage}</Text>}
      <Group>
