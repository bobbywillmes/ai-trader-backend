...
export function TradingAccountsPage() {
  ...
  const action = (account: TradingAccount) => <Button size="compact-sm" variant="default" onClick={() => openAccount(account)}>Open account</Button>;
  ...
  const narrow = (items: readonly TradingAccount[]) => <MobileRecordCard records={items} getRecordId={(account) => account.id} renderIdentity={identity} renderStatus={(account) => <StatusBadge status={account.environment} tone={account.environment === "LIVE" ? "danger" : "informational"} size="compact" />} renderFields={(account) => [{ label: "Broker", value: account.broker }, ...fields(account)]} onDetails={(account) => openAccount(account)} detailsLabel="Open account" detailsIsDialog={false} />;
