...
describe("Trading Accounts responsive page", () => {
  ...
  it("renders mobile cards and navigates to the correct account", async () => { renderPage(); resize?.(390); await waitFor(() => expect(screen.getByLabelText("Trading accounts").getAttribute("data-presentation")).toBe("narrow")); const buttons = screen.getAllByRole("button", { name: "Open account" }); expect(screen.queryByRole("table")).toBeNull(); expect(screen.getByLabelText("Live status")).toBeTruthy(); expect(buttons[0].getAttribute("aria-haspopup")).toBeNull(); await userEvent.setup().click(buttons[1]); await waitFor(() => expect(mocks.navigate).toHaveBeenCalledWith("/trading-accounts/2")); });
