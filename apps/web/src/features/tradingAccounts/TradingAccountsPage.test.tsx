...
const mocks = vi.hoisted(() => ({
  ...
  navigate: vi.fn(), owner: true, accountUser: false,
}));
...
vi.mock("../auth/useAuth", () => ({ useIsSystemOwner: () => mocks.owner, useIsAccountUser: () => mocks.accountUser }));
vi.mock("../../lib/api", () => ({ getAdminToken: () => "token" }));
vi.mock("react-router-dom", () => ({ useNavigate: () => mocks.navigate, useLocation: () => ({ search: "?account=2" }) }));
...
afterEach(() => { cleanup(); vi.unstubAllGlobals(); mocks.accountUser = false; });
...
describe("Trading Accounts responsive page", () => {
  ...
  it("renders mobile cards and preserves operational scope when opening an account", async () => { renderPage(); resize?.(390); await waitFor(() => expect(screen.getByLabelText("Trading accounts").getAttribute("data-presentation")).toBe("narrow")); const buttons = screen.getAllByRole("button", { name: "View account" }); expect(screen.queryByRole("table")).toBeNull(); expect(screen.getByLabelText("Live status")).toBeTruthy(); expect(buttons[0].getAttribute("aria-haspopup")).toBeNull(); await userEvent.setup().click(buttons[0]); await waitFor(() => expect(mocks.navigate).toHaveBeenCalledWith("/trading-accounts/1?account=2")); });
  ...
  it("uses the shared My Accounts presentation for Account Users", () => { mocks.accountUser = true; mocks.owner = false; renderPage(); expect(screen.getByRole("heading", { name: "My Accounts" })).toBeTruthy(); expect(screen.queryByRole("button", { name: "New Trading Account" })).toBeNull(); });
