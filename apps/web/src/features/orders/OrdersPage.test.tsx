...
const mocks = vi.hoisted(() => ({
  query: { data: [] as OpenOrder[], isLoading: false, isError: false, error: null as Error | null, refetch: vi.fn() },
  mutateAsync: vi.fn(), pending: false, variables: undefined as { orderId: string } | undefined, confirm: vi.fn(), notify: vi.fn(),
}));
vi.mock("./hooks", () => ({ useOpenOrders: () => mocks.query, useCancelOrder: () => ({ mutateAsync: mocks.mutateAsync, isPending: mocks.pending, variables: mocks.variables }) }));
...
describe("Open Orders responsive page", () => {
  ...
  it("expands compact details for the correct order and keeps routing collapsed", async () => {
    renderPage(); resize?.(800); await screen.findByText("2 open orders");
  ...
  it("opens the correct mobile drawer, closes on Escape, and restores focus", async () => {
    renderPage(); resize?.(390); await screen.findByText("2 open orders");
  ...
  it("renders shared loading, empty, and recoverable error states", async () => {
    mocks.query.data = []; mocks.query.isLoading = true; const view = renderPage(); expect(screen.getByRole("status")).toBeTruthy();
