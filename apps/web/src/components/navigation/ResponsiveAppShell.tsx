...
function SidebarLink({ item, expanded, onNavigate, preserveTradingAccountScope, nested = false }: { item: AdminNavItem; expanded: boolean; onNavigate?: () => void; preserveTradingAccountScope: boolean; nested?: boolean }) {
  ...
  const link = <UnstyledButton className={`${classes.navLink} ${nested ? classes.nestedLink : ""}`} data-active={active || undefined} aria-current={active ? "page" : undefined} onClick={() => { navigate(target); window.scrollTo({ top: 0, left: 0, behavior: "auto" }); onNavigate?.(); }}><Icon size={nested ? 17 : 21} stroke={1.8} aria-hidden="true" /><span className={expanded ? classes.linkLabel : classes.visuallyHidden}>{item.label}</span>{item.badge && <Badge color={item.badge.color} size="xs" aria-label={item.badge.label}>{item.badge.count > 99 ? "99+" : item.badge.count}</Badge>}{expanded && !nested && <IconChevronRight className={classes.linkChevron} size={15} aria-hidden="true" />}</UnstyledButton>;
