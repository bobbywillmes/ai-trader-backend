...
type Props = { state: "loading" | "empty" | "error"; title?: string; message?: string; onRetry?: () => void };

export function DataState({ state, title, message, onRetry }: Props) {
  ...
  return <Center py="xl"><Stack align="center" gap="xs"><IconDatabaseOff aria-hidden="true" /><Text fw={600}>{title ?? "No records"}</Text><Text c="dimmed" size="sm">{message ?? "There is nothing to show yet."}</Text></Stack></Center>;
}
