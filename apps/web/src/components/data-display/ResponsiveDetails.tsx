...
export function ResponsiveDetails({ opened, title, children, onClose, returnFocusTo, mode = "drawer" }: Props) {
  ...
  return <Drawer opened={opened} onClose={onClose} title={title} position="right" size="min(var(--data-drawer-width), 100vw)" closeOnEscape closeOnClickOutside trapFocus lockScroll aria-label={typeof title === "string" ? title : "Record details"} styles={{ content: { maxWidth: "100%" } }}>{children}</Drawer>;
}
