...
type Props<T> = {
  ...
  detailsLabel?: string;
  detailsIsDialog?: boolean;
};

export function MobileRecordCard<T>({ records, getRecordId, renderIdentity, renderStatus, renderFields, onDetails, renderActions, detailsLabel = "View details", detailsIsDialog = true }: Props<T>) {
  return <div className={classes.cards}>{records.map((record) => <article className={classes.card} key={getRecordId(record)} data-record-id={getRecordId(record)}>
    ...
    <div className={classes.cardActions}><Button className={classes.detailButton} variant="default" onClick={(event) => onDetails(record, event.currentTarget)} aria-haspopup={detailsIsDialog ? "dialog" : undefined}>{detailsLabel}</Button>{renderActions?.(record)}</div>
  </article>)}</div>;
}
