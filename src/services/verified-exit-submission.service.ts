...
export type VerifiedExitContext = {
  verificationMode?: "NORMAL_EXACT_CLOSE" | "CORRECTIVE_REMAINDER_CLOSE";
  ...
  attributedExitFilledQty?: number | string;
...
type ConflictingOrderEvidence = {
  type: string; side: string; status: string; qty: string | null; filledQty: string | null;
  remainingQty: string | null; limitPrice: string | null; stopPrice: string | null;
...
async function executeVerifiedExit(
  context: VerifiedExitContext,
): Promise<VerifiedExitSubmissionResult> {
  ...
  if (localOrder)
    return { outcome: "RECOVERED_LOCAL", brokerOrderId: localOrder.id };
  ...
  try {
    ...
    return block({
      ...
      outcome: "BROKER_STATE_UNAVAILABLE",
      message: `${symbol} deterministic exit recovery could not be verified.`,
      evidence: {
        failureClassification: "CLIENT_ORDER_ID_LOOKUP_FAILED",
        error:
  ...
  try {
    position = await getAlpacaPositionBySymbol(
      context.tradingAccountId,
      symbol,
      "manual_admin_action",
    );
    ...
    return block({
      ...
      environment: account.environment,
      outcome: "BROKER_STATE_UNAVAILABLE",
      message: `${symbol} broker position lookup failed; no sell was submitted.`,
      evidence: {
        failureClassification: "POSITION_LOOKUP_FAILED",
        error:
          error instanceof Error ? error.message : "Unknown lookup failure.",
      },
    });
  ...
  try {
    openOrders = await getOpenAlpacaOrders(
      context.tradingAccountId,
      "manual_admin_action",
    );
  } catch (error) {
    return block({
      ...
      environment: account.environment,
      outcome: "BROKER_STATE_UNAVAILABLE",
      message: `${symbol} open-order lookup failed; no sell was submitted.`,
      evidence: {
        failureClassification: "OPEN_ORDER_LOOKUP_FAILED",
        error:
          error instanceof Error ? error.message : "Unknown lookup failure.",
      },
    });
  ...
  if (!position)
    return block({
      ...
      message: `${symbol} is absent at the broker; no sell was submitted.`,
      evidence: {
        brokerPositionSide: null,
        brokerHeldQty: null,
  ...
  if (position.symbol.toUpperCase() !== symbol || position.side !== "long") {
    ...
    return block({
      ...
      message:
        position.side === "short"
          ? `${symbol} is short at the broker. All sell automation is blocked.`
          : `${symbol} broker position identity is ambiguous.`,
      evidence: {
        brokerAssetId: position.asset_id,
        brokerSymbol: position.symbol,
        brokerPositionSide: position.side,
        brokerHeldQty: position.qty,
        brokerAvailableQty: position.qty_available ?? null,
      },
    });
  }
  ...
  if (!held || !available || !intended || !local)
    return block({
      context,
      environment: account.environment,
      outcome: "BROKER_STATE_UNAVAILABLE",
      message: `${symbol} quantity evidence is missing or malformed; no sell was submitted.`,
      evidence: {
        brokerPositionSide: position.side,
        brokerHeldQty: position.qty,
        brokerAvailableQty: position.qty_available ?? null,
  ...
  const conflicts = openOrders
    .filter(
      (order) =>
        ...
        (context.verificationMode === "CORRECTIVE_REMAINDER_CLOSE" ||
          order.side === "sell") &&
        isNonterminalBrokerOrderStatus(order.status) &&
        order.client_order_id !== context.clientOrderId,
    )
    .map((order) => {
  ...
  if (conflicts.some((order) => order.remainingQty === null))
    return block({
      context,
      environment: account.environment,
      outcome: "BROKER_STATE_UNAVAILABLE",
      message: `${symbol} has an active sell order with ambiguous remaining quantity.`,
      evidence: {
        brokerPositionSide: position.side,
        brokerHeldQty: held.canonical,
        brokerAvailableQty: available.canonical,
        conflictingActiveSellOrders: conflicts,
      },
    });
  if (conflicts.length)
    return block({
      ...
      evidence: {
        ...
        conflictingActiveSellOrders: conflicts,
        nextAction:
          "Review or cancel the active order(s), or allow them to complete, then retry.",
      },
    });
  const corrective = context.verificationMode === "CORRECTIVE_REMAINDER_CLOSE";
  const attributed = corrective
    ? parseExactPositiveDecimal(context.attributedExitFilledQty)
    : null;
  const expectedRemainder =
    corrective && attributed ? subtractDecimal(local, attributed) : null;
  const quantityVerified = corrective
    ? attributed !== null &&
      expectedRemainder !== null &&
      expectedRemainder !== "0" &&
      equalDecimal(intended, parseExactPositiveDecimal(expectedRemainder)!) &&
      equalDecimal(held, intended)
    : equalDecimal(held, intended) && equalDecimal(held, local);
  if (!quantityVerified)
    return block({
      context,
      environment: account.environment,
      ...
      message: corrective
        ? `${symbol} corrective lifecycle equation could not be verified exactly.`
        : `${symbol} held, local, and intended quantities do not match exactly.`,
      evidence: {
        verificationMode: context.verificationMode ?? "NORMAL_EXACT_CLOSE",
        attributedExitFilledQty: attributed?.canonical ?? null,
        expectedRemainder,
        brokerPositionSide: position.side,
        brokerHeldQty: held.canonical,
        brokerAvailableQty: available.canonical,
  ...
  if (compareDecimal(available, held) > 0)
    return block({
      context,
      environment: account.environment,
      outcome: "RESERVED_QUANTITY",
      message: `${symbol} broker-available quantity exceeds held quantity; no sell was submitted.`,
      evidence: {
        brokerPositionSide: position.side,
        brokerHeldQty: held.canonical,
        brokerAvailableQty: available.canonical,
        failureClassification: "AVAILABLE_QUANTITY_EXCEEDS_HELD",
      },
    });
  if (!equalDecimal(held, available))
    return block({
      context,
      environment: account.environment,
      outcome: "RESERVED_QUANTITY",
      message: `${symbol} broker-available quantity is lower than held quantity; no sell was submitted.`,
      evidence: {
        brokerPositionSide: position.side,
        brokerHeldQty: held.canonical,
        brokerAvailableQty: available.canonical,
  ...
  const clearedAttentions =
    context.verificationMode === "CORRECTIVE_REMAINDER_CLOSE"
      ? []
      : await prisma.operationalAttention.findMany({
          where: {
            tradingAccountId: context.tradingAccountId,
  ...
  for (const attention of clearedAttentions) {
    await resolveOperationalAttentionAuthoritatively({
      ...
      reason:
        "A complete fresh broker verification no longer observed this exit-safety condition.",
      evidence: {
  ...
  await authorizeLiveBrokerWrite(
    context.tradingAccountId,
    "RISK_REDUCING_WRITE",
  );
  const payload: VerifiedAlpacaExitOrderRequest = {
    ...
    ...(context.order.type === "trailing_stop"
      ? { trail_percent: context.order.trailPercent }
      : {}),
  };
  ...
  try {
    ...
    if (
      ...
      error.classification === "BROKER_REJECTED"
    ) {
      try {
        ...
        if (recoveredAfterRejection)
          return {
            outcome: "RECOVERED_BROKER",
            order: recoveredAfterRejection,
          };
      } catch {
        throw new BrokerWriteDeliveryError({
          classification: "DELIVERY_UNCERTAIN",
          message:
            "Broker rejection could not be confirmed because deterministic recovery failed.",
          cause: error,
        });
      }
    }
...
/** Corrective workflow entrypoint for callers that already own the same account-scoped EXIT_SUBMISSION lock. */
export async function submitVerifiedCorrectiveExitWithinAccountLock(
  context: VerifiedExitContext & {
    verificationMode: "CORRECTIVE_REMAINDER_CLOSE";
    attributedExitFilledQty: number | string;
  },
) {
  return executeVerifiedExit(context);
}
