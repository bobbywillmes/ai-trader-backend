...
const TRADING_ACCOUNT_ADMIN_SELECT = {
  ...
  activeLiveEntryArming: {
    select: {
      id: true,
      entryApprovalRevision: true,
      tradingAccountSubscriptionId: true,
      entryApprovalExpiresAt: true,
      armedAt: true,
      terminations: { select: { type: true, occurredAt: true } },
    },
  },
  liveEntryArmings: {
    orderBy: { id: 'desc' },
...
export function serializeTradingAccountForAdmin(
  ...
  return {
    ...
    activeLiveEntryArming: account.activeLiveEntryArming,
    latestLiveEntryArming: account.liveEntryArmings?.[0] ?? null,
